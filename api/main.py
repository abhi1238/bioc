...
# from api.routers import health, ws, query
# import os
...
#         app.state.embedder_client = EmbeddingClient(settings=settings, logger=logger)
#         logger.info("[startup] PID=%s: embedder client loaded", pid)
#     except Exception as e:
#         logger.exception("[startup] PID=%s: embedder client failed to load: %s", pid, e)
...
@asynccontextmanager
async def lifespan(app: FastAPI):
    app.state.embedder_client = EmbeddingClient(settings=settings)
    yield
